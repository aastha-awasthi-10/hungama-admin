...
const Ads = (props) => {
  ...
  const pageData = async (page = activepage) => {
    ...
    path = apiUrl.get_ads + `?${queryString}`;
    getData(path)
  };
  ...
  const handleSearching = async () => {
    ...
    setQuery(queries)
    path = apiUrl.get_ads + '?page=' + `${page}` + '&itemsPerPage=' + `${itemsPerPage}` + '&keyword=' + `${keywords}` + '&start_date=' + `${startdate == '' ? '' : moment(startdate).format('YYYY-MM-DD')}` + '&end_date=' + `${enddate == '' ? '' : moment(enddate).format('YYYY-MM-DD')}` + '&status=' + `${serachstatus}`;
    getData(path)
  };

  const resetSearch = async () => {
    let path = apiUrl.get_news + '?page=1&itemsPerPage=10';
    getData(path)
  }
          ...
          <Card>
            <CardHeader className="align-items-center d-flex">
              ...
              <div className="ml-auto">
                <Link to="/add-ads" className="btn btn-primary">
                  <i className="fa fa-plus mr-1"></i>Add Ads
                </Link>
              </div>
            </CardHeader>
            <CardBody>
                    ...
                    {users.map((item, key) => {
                      return (
                        <tr key={key}>
                          ...
                          <td className="text-left">{_.upperFirst(item.type)}</td>
                          ...
                          <td className="text-center" className="w-40">
                            {
                              _.isEmpty(props.match.params.id && true) ?
                                <div>
                                  ...
                                  <Link to={{ pathname: `/edit-ads/${item.id}` }} className="btn-link">
