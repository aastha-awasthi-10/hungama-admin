...
const Questions = (props) => {
  ...
  const handleSearching = async () => {
    ...
    setQuery(queries)
    path = apiUrl.get_questions + '?page=' + `${page}` + '&itemsPerPage=' + `${itemsPerPage}` + '&keyword=' + `${keywords}` + '&language=' + `${language}` + '&quizCat=' + `${category_id}` + '&start_date=' + `${startdate == '' ? '' : moment(startdate).format('YYYY-MM-DD')}` + '&end_date=' + `${enddate == '' ? '' : moment(enddate).format('YYYY-MM-DD')}` + '&status=' + `${serachstatus}`;
    getData(path)
  };


  const resetSearch = async () => {
    let path = apiUrl.get_questions + '?page=1&itemsPerPage=10';
    getData(path)
  }
            ...
            <CardBody>
                ...
                <Row>
                  <Col lg={9}>
                    <Form>
                      <Row>
                        ...
                        <Col md={2} sm={6}>
                          <FormGroup className="mb-xl-0">
                            <select name={'language'} className={"form-control"} value={language} onChange={(e) => { setLanguage(e.target.value); }} >
                  ...
                  <Col lg={6}>
                    <Row>
                      ...
                      <Col md={3}>
                        <div>
                          <Link to={{ pathname: `/import-questions/` }} className="btn-link">
                            <button className="btn btn-primary mr-1 col-md-12" type="button" title="Import Excel">
                              <i className="fa fa-file-excel-o" /> Import Excel
                            </button>
                          </Link>
                        </div>
                      </Col>
              ...
              <div id="reportId" >
                <Table hover bordered responsive className="mt-3 text-center">
                  <thead>
                    <tr>
                      <th className="text-left">Question Mode</th>
                  ...
                  <tbody>
                    {users.map((item, key) => {
                      return (
                        <tr key={key}>
                          <td className="text-left">{item.question_mode}</td>
                          <td className="text-left">{(item.question_mode == 'quiz') ? item.quiz_id.name : item.contest_id.name}</td>
                          <td className="text-left">{item.category_id.title}</td>
                          <td className="text-left">{_.upperFirst(item.question_type)}</td>
                          <td className="text-right">{item.question_time}</td>
                          <td className="text-left">{item.question_language == 'english_hindi' ? _.upperFirst('hindi & english') : _.upperFirst(item.question_language)}</td>
                          ...
                          <td className="text-center" className="w-40">
                            {
                              _.isEmpty(props.match.params.id && true) ?
                                <div>
                                  ...
                                  <Link to={{ pathname: `/edit-question/${item.id}` }} className="btn-link">
