...
const UserQuizMatches = (props) => {
    ...
    const handleSearching = async () => {
        ...
        setQuery(queries)
        path = apiUrl.get_quiz_match + '?id=' + `${props.match.params.id}` + '&page=' + `${page}` + '&itemsPerPage=' + `${itemsPerPage}` + '&keyword=' + `${keywords}` + '&quizCat=' + `${category_id}` + '&quiz_start_date=' + `${startdate == '' ? '' : moment(startdate).format('YYYY-MM-DD')}` + '&quiz_end_date=' + `${enddate == '' ? '' : moment(enddate).format('YYYY-MM-DD')}` + '&status=' + `${serachstatus}`;
        getData(path)
    };
...
export default UserQuizMatches;
