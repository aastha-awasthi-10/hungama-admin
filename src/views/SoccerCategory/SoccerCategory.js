...
const SoccerCategory = (props) => {
  ...
  const pageData = async (page = activepage) => {
    ...
    path = apiUrl.get_soccer_categories + `?${queryString}`;
    getData(path)
  };
  ...
  const handleSearching = async () => {
    ...
    setQuery(queries)    
    path = apiUrl.get_soccer_categories + '?page=' + `${page}` + '&itemsPerPage=' + `${itemsPerPage}` + '&keyword=' + `${keywords}` + '&start_date=' + `${startdate == '' || startdate == null ? '' : moment(startdate).format('YYYY-MM-DD')}` + '&end_date=' + `${enddate == '' || enddate == null ? '' : moment(enddate).format('YYYY-MM-DD')}` + '&status=' + `${serachstatus}`;
    getData(path)    
  };

  const resetSearch = async () => {
    let path = apiUrl.get_soccer_categories + '?page=1&itemsPerPage=10';
    getData(path)
  }  
          ...
          <Card>
            ...
            Soccer Categories
              <div className="ml-auto">
                <Link to="/soccer/add-category" className="btn btn-primary">           
                  <i className="fa fa-plus mr-1"></i>Add Soccer Category
                </Link>   
              </div>
            </CardHeader>
            <CardBody>
                    ...
                    {users.map((item, key) => {
                      return (
                        <tr key={key}>
                          ...
                          <td className="text-right">{item.sequence}</td>
                          <td className="text-left"><img src={item.image} height="100" width="200" /></td>
                          ...
                          <td className="text-center" className="w-40">
                            {
                              _.isEmpty(props.match.params.id && true) ?
                                <div>
                                  <Link to={{ pathname: `/soccer/edit-category/${item.id}` }} className="btn-link">
