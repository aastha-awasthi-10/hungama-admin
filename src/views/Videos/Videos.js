...
const Videos = (props) => {
  ...
  const pageData = async (page = activepage) => {
    ...
    path = apiUrl.get_video + `?${queryString}`;
    getData(path)
  };
  ...
  const handleSearching = async () => {
    ...
    setQuery(queries)
    path = apiUrl.get_video + '?page=' + `${page}` + '&itemsPerPage=' + `${itemsPerPage}` + '&keyword=' + `${keywords}` + '&start_date=' + `${startdate == '' ? '' : moment(startdate).format('YYYY-MM-DD')}` + '&end_date=' + `${enddate == '' ? '' : moment(enddate).format('YYYY-MM-DD')}` + '&status=' + `${serachstatus}`;
    getData(path)
  };

  const resetSearch = async () => {
    let path = apiUrl.get_video + '?page=1&itemsPerPage=10';
    getData(path)
  }
            ...
            <CardHeader className="align-items-center d-flex">
              ...
              <div className="ml-auto">
                <Link to="/add-video" className="btn btn-primary">
                  <i className="fa fa-plus mr-1"></i>Add Video
                </Link>
              </div>
            </CardHeader>
            <CardBody>
                          ...
                          <td className="text-center" className="w-40">
                            {
                              _.isEmpty(props.match.params.id && true) ?
                                <div>
                                  ...
                                  <Link to={{ pathname: `/edit-video/${item.id}` }} className="btn-link">
