...
const Result = (props) => {
  ...
  const winning_distribute = async (match_id, series_id) => {
    ...
    let path = apiUrl.series_completed_matches_distribution;
  ...
  const refresh_rank = async (match_id, series_id) => {
    ...
    let path = apiUrl.refresh_series_completed_matches_rank;
    const fr = await Helper.post(postJson, path, token);
    const res = await fr.response.json();
    if (fr.status === 200) {
      if (res.success) {
              ...
              <div className="multipal-searching">
                <Row>
                  ...
                  <Col lg={2}>
                    <Row>
                      <Col md={12}>
                        <button className="btn btn-primary mr-1 col-md-5 d-inline-flex align-items-center" type="button" onClick={handleSearching}><i className="fa fa-search" /> Search{isserach === true && <i className="fa fa-spinner fa-pulse fa-fw ml-1" />}</button>
                        ...
                        <tr key={key}>
                          ...
                          <td className="text-right">
                            {((item.win_flag == 0) && (item.show_win_amount == true)) ?
                              <button className={'btn circle_btn table_auto_btn mr-5 '} type={'button'} onClick={(e) => { winning_distribute(item.match_id, item.series_id) }} > Winning Distribute</button>
                              : ((item.win_flag == 1) && (item.show_win_amount == true))
                                ? <span className={'text-success font-weight-bold mr-5'}> Distributed </span>
                                : <span className={'mr-5 status_text'}> No Amount Yet </span>
                            }
                            <button className={'btn circle_btn table_auto_btn'} type={'button'} onClick={(e) => { refresh_rank(item.match_id, item.series_id) }}><i className="fa fa-undo fa-fw ml-1"></i> Refresh</button>
