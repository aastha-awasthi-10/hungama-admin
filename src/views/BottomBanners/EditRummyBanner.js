...
const EditRummyBanner = (props) => {
  ...
  const onSubmit = async data => {
    ...
    if (fr.status === 200) {
      if (res.success) {
        setLoading(false);
        props.history.push('/edit-rummy-banner');
  ...
  const getData = async () => {
    ...
    let path = apiUrl.get_bottom_banner_by_type + '/' + `rummy`;
    const fr = await Helper.get(path, token);
    const res = await fr.response.json();
    if (fr.status === 200) {
      if (res.success) {
  ...
  return (
    <React.Fragment>
      <form onSubmit={handleSubmit(onSubmit)} encType="multipart/form-data">
        <Card>
          <CardHeader>
            <CardTitle className=""><h4>Edit Rummy Banner</h4></CardTitle>
          </CardHeader>
          <CardBody>
            <Row>
              <Col md={12}>
...
export default EditRummyBanner;
