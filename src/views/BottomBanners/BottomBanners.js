...
const BottomBanners = (props) => {
  ...
  const pageData = async (page = activepage) => {
    ...
    path = apiUrl.get_bottom_banners + `?${queryString}`;
    getData(path)
  };
  ...
  const handleSearching = async () => {
    ...
    setQuery(queries)
    path = apiUrl.get_bottom_banners + '?page=' + `${page}` + '&itemsPerPage=' + `${itemsPerPage}` + '&keyword=' + `${keywords}` + '&start_date=' + `${startdate == null || startdate == "" ? '' : moment(startdate).format('YYYY-MM-DD')}` + '&end_date=' + `${enddate == null || enddate == "" ? '' : moment(enddate).format('YYYY-MM-DD')}` + '&status=' + `${serachstatus}`;
    getData(path)
  };

  const resetSearch = async () => {
    let path = apiUrl.get_bottom_banners + '?page=1&itemsPerPage=10';
    getData(path)
  }
          ...
          <Card>
            <CardHeader className="align-items-center d-flex">
              ...
              {/* <div className="ml-auto">
                <Link to="/add-bottom-banner" className="btn btn-primary">
                  <i className="fa fa-plus mr-1"></i>Add Bottom Banner
                </Link>
              </div> */}
            </CardHeader>
            <CardBody>
                      ...
                      return (
                        <tr key={key}>
                          ...
                          <td className="text-align-center">
                            {(item.media_type == 'image') &&  ((item.image !=='') ? <img src={bannerPath + item.image} height="100" width="200" /> : <img src={bannerPath + 'avtar.png'} height="100" width="200" />)}
                          ...
                          <td className="text-align-center w-40">
                            {
                              _.isEmpty(props.match.params.id && true) ?
                                <div>
                                  ...
                                  {((user_type === "editor" && module.delete == 1) || user_type=="admin")?<Delete item={item} refreshData={pageData} />:null}
                                  {((user_type === "editor" && module.edit == 1) || user_type=="admin")?                                  
                                  <Link to={{ pathname: `/edit-bottom-banner/${item.id}` }} className="btn-link">
