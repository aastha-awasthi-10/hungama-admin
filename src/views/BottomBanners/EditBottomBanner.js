...
const EditBottomBanner = (props) => {
  ...
  const onSubmit = async data => {
    ...
    if (fr.status === 200) {
      if (res.success) {
        setLoading(false);
        props.history.push('/edit-cricket-banner');
  ...
  const getData = async () => {
    ...
    let path = apiUrl.get_bottom_banner_by_type + '/' + `match`;
    const fr = await Helper.get(path, token);
    const res = await fr.response.json();
    if (fr.status === 200) {
      if (res.success) {
    ...
    const fr2 = await Helper.get(path, token);
    const res2 = await fr2.response.json();

    if (fr2.status === 200) {

      if (res2.success) {
        setDropdown(res2.results || []);
        setVisibale(false);
      } else {
            ...
            <Row>
              ...
              <Col md={12}>
                <FormGroup>
                  ...
                  <span className={""} >
                    (Please enter only .png, .jpg .gif and .jpeg images with dimension 600 X 400 and max size of 12MB for best view.) <br></br>
