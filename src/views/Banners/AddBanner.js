...
const AddBanner = (props) => {
  ...
  const [defaultimage, setDefaultImage] = useState(process.env.REACT_APP_IMAGE_BASE_URL + '/static/front_home/video.png');
  ...
  const onImageChange = (event) => {
    if (event.target.files && event.target.files[0]) {
      ...
      // if(mediaType == 'video'){
      //     if (!image.name.match(/\.(x-msvideo|mpeg|mp4|ogg|webm)$/)) {
      //         Toast.fire({
      //             type: "error",
      //             title: "select valid video file.",
      //         });
      //         setButtonState(true)
      //         return false;
      //     }else{
              ...
              <Col md={6}>
                <FormGroup>
                  ...
                  {mediaType === 'video' ?
                    ...
                    : <span className={"pull-left"} > (Please enter only .png, .jpg .gif and .jpeg images with dimension 600 X 400 and max size of 2MB for best view.) </span>
                  }
