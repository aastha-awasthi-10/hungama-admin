...
useEffect(() => {
 ...
 const func = (url) => {
  ...
  img.src = url;
  if (img.complete) {
    return <td className="text-left"><img width="40px" src={url} /></td>
  } else {
    return <td className="text-left"><img width="40px" src={process.env.REACT_APP_IMAGE_BASE_URL +
      "/static/team_flags/no_image.jpg"} /></td>
  }
 }
...
// }
// // USAGE
// checkIfImageExists(process.env.REACT_APP_IMAGE_BASE_URL + '/static/team_flags/'+item.flag, (exists) => {
//   if (exists) {
//     <td className="text-left"><img width="40px" src={process.env.REACT_APP_IMAGE_BASE_URL + '/static/team_flags/'+item.flag} /></td>
//   } else {
//     <td className="text-left">not working</td>
//   }
// });
  return (
                    ...
                    {teams.map((item, key) => {
                      ...
                      return (
                        <tr key={key}>
                          ...
                          {/* {func(process.env.REACT_APP_IMAGE_BASE_URL + '/static/team_flags/'+item.flag)} */}
                          <td className="text-left"><img width="40px" src={process.env.REACT_APP_IMAGE_BASE_URL + '/static/team_flags/'+item.flag} /></td>
                              ...
                              _.isEmpty(item.team_id && true) ?
                                <div>
                                  <Link to={{ pathname: `/edit-flag/${item.team_id}` }} className="btn-link">
                                    <button className="btn circle_btn table_auto_btn btn-sm mr-1" type="button" title="Update Short Name">
                                      Update Flag
                                    </button>
                                  </Link>
                                </div>
