...
const EditBotUser = (props) => {
    ...
    const onSubmit = async data => {
        ...
        let postJson = { id: id, gender: gender, dob: dob, first_name: data.first_name.trim(), last_name: data.last_name.trim(), email: data.email, username: _.snakeCase(data.username), phone: data.mobile };
        formData.append('data', JSON.stringify(postJson));
        formData.append('profile_pic', profilePic);
        let path = apiUrl.update_bot_user;
        const fr = await Helper.formPost(formData, path, token);
        const res = await fr.response.json();
        if (fr.status === 200) {
            if (res.success) {
                setLoading(false);
                props.history.push('/buser');
                        ...
                        <Row>
                            <Col md={6}>
                                <FormGroup>
                                    ...
                                    <input type="text" minLength={"3"} maxLength={"50"} name="first_name" placeholder="First Name" autoComplete="off"
                                        className="form-control col-md-8" defaultValue={UserData.first_name} ref={register({ required: 'Required', pattern: /^[A-Za-z]+$/i })} />
                                    {errors.first_name && <p className="text-danger marginmessage">First Name is required and should be alphabatic only</p>}
                                </FormGroup>
                            </Col>
                            <Col md={6}>
                                <FormGroup>
                            ...
                            <Col md={6}>
                                <FormGroup>
                                    ...
                                    <input type="text" name="username" disabled id={"username"} defaultValue={UserData.username} className="form-control col-md-8" autoComplete="off" placeholder="User Name"
                            ...
                            <Col md={6}>
                                <FormGroup>
                                    ...
                                    <input type="text" name="mobile" disabled className="form-control  col-md-8" defaultValue={UserData.phone} maxLength="10" placeholder="Phone Number"
                                        ref={register({
                            ...
                            <Col md={6}>
                                <FormGroup>
                                    ...
                                    <input type="text" name="email" disabled className="form-control  col-md-8" defaultValue={UserData.email} placeholder="Email"
                                        ref={register({
