...
const Contests = (props) => {
  ...
  const handleSearching = async () => {
    ...
    setQuery(queries)
    path = apiUrl.get_constest + '?page=' + `${page}` + '&itemsPerPage=' + `${itemsPerPage}` + '&keyword=' + `${keywords}` + '&contestType=' + `${contestType}` + '&quizCat=' + `${category_id}` + '&start_date=' + `${startdate == '' ? '' : moment(startdate).format('YYYY-MM-DD')}` + '&end_date=' + `${enddate == '' ? '' : moment(enddate).format('YYYY-MM-DD')}` + '&status=' + `${serachstatus}`;
    getData(path);
  };
          ...
          <Card>
            <CardHeader className="align-items-center d-flex">
              ...
              <div className="ml-auto">
                <Link to="/add-contest" className="btn btn-primary">
                  <i className="fa fa-plus mr-1"></i>Add Contest
                </Link>
              </div>
            </CardHeader>
            <CardBody>
                ...
                <Table hover bordered responsive className="mt-3 text-center">
                  <thead>
                    <tr>
                      ...
                      <th className="text-right">Joined Users</th>
                      ...
                      <th className="text-left">Contest Language</th>
                    ...
                    {users.map((item, key) => {
                      return (
                        <tr key={key}>
                          ...
                          <td className="text-center">{moment(item.start_date).format('lll')}</td>
                          <td className="text-center">{moment(item.end_date).format('lll')}</td>
                          <td className="text-right">{item.difficulty_level}</td>
                          <td className="text-left">{_.upperFirst(item.contest_type)}</td>
                          <td className="text-left">{item.contest_language == 'english_hindi' ? _.upperFirst('hindi & english') : _.upperFirst(item.contest_language)}</td>
                          <td className="text-right">{item.entry_fee}</td>
                          <td className="text-right">{item.winning_amount}</td>
                          <td className="text-center">
                            {item.status != 'inactive' ? _.upperFirst(item.status) : <Status item={item} refreshData={pageData} />}
                          </td>
                              ...
                              _.isEmpty(props.match.params.id && true) ?
                                item.status == 'inactive' ?
                                  <div>
                                    ...
                                    <Link to={{ pathname: `/edit-contest/${item.id}` }} className="btn-link">
