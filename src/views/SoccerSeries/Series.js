...
const Series = (props) => {
  ...
  const pageData = async (page = activepage) => {
    ...
    path = apiUrl.get_soccer_series_contest + `?${queryString}`;
    getData(path)
  }; 
  ...
  const handleSearching = async () => {
    ...
    path = apiUrl.get_soccer_series_contest + '?page=' + `${page}` + '&itemsPerPage=' + `${itemsPerPage}` + '&keyword=' + `${keywords}` + '&status=' + `${serachstatus}`;
    getData(path)    
  };

  const resetSearch = async () => {
    let path = apiUrl.get_soccer_series_contest + '?page=1&itemsPerPage=10';
    getData(path)
  }  
                          ...
                          <td className="text-center" className="w-40">
                            {
                              _.isEmpty(props.match.params.id && true) ?
                                <div>
                                  <Link to={{ pathname: `/soccer-update-short-name/${item.rows.id_api}` }} className="btn-link">
