...
const View = (props) => {
  ...
  const getSeriesMatches = async () => {
    ...
    path = apiUrl.get_soccer_series_all_matches + '/' + props.item.id_api;
    console.log(path);
  ...
  return (
    <div className={"inline-btn"}>
      <Button className="btn circle_btn btn-sm mr-1" type="button" title="View Details" onClick={handleClick}>Matches</Button>
              ...
              {matches.map((item, key) => {
                return (
                  <tr key={key}>
                    ...
                    <td>
                      {
                        <MatchStatus item={item} refreshStatusData={getSeriesMatches} />
